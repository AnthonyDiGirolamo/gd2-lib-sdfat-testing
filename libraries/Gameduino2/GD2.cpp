...
#define SD_PIN        20     // pin used for the microSD enable signal
#endif
...
byte GDClass::load(const char *filename, void (*progress)(long, long)) {
  ...
  uint8_t buf[512];
  int readsize;
  while ((readsize = file.read(buf, 512)) > 0) {
    // Check first four bytes of each read
    // for (int i=0; i<4; i++) {
    //   Serial.print(buf[i], HEX);
    // }
    // Serial.println();
    GDTR.cmd_n(buf, (readsize + 3) & ~3);
  }
  file.close();
  return 1;
#else
